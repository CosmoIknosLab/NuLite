...
#
# Original @ Fabian Hörst, fabian.hoerst@uk-essen.de
# Institute for Artifical Intelligence in Medicine,
# University Medicine Essen

# Adapted @ Cristian Tommasino cristian.tommsino@unina.it
#
#
import argparse
...
from nuclei_detection.utils.post_proc_nulite import calculate_instances
from utils.logger import Logger
from utils.tools import DataclassHVStorage
...
from nuclei_detection.datasets.dataset_coordinator import (select_dataset)
from nuclei_detection.utils.metrics import (
...
from nuclei_detection.utils.tools import cropping_center, pair_coordinates


class InferenceNuLite:
    def __init__(
            self,
        ...
        self.save_dir = self.run_dir / f"{checkpoint_name.split('.')[0]}_{'reparameterize' if reparameterize == True else 'no_reparameterize'}"
    ...
    def __load_run_conf(self) -> None:
        """Load the training_example.yaml file with the run setup
        ...
        """
        with open((self.run_dir / "config_0.yaml").resolve(), "r") as run_config_file:
    ...
    def __instantiate_logger(self) -> None:
        ...
        logger = Logger(
            ...
            log_dir=Path(self.save_dir).resolve(),
            comment="inference",
            use_timestamp=False,
            formatter="%(message)s",
        )
    ...
    def setup_patch_inference(
        ...
        Args:
            test_folds (List[int], optional): Test fold to use. Otherwise defined folds from training_example.yaml (in run_dir) are loaded. Defaults to None.
        ...
        if checkpoint["arch"] == "CellFastViT" and self.reparameterize is True:
            model.eval()
            model.reparameterize_encoder()
            self.logger.info("Model reparameterized")
        # get dataset
    ...
    def run_patch_inference(
        ...
        # saving
        with open(str(self.save_dir / "inference_results.json"), "w") as outfile:
    ...
    def inference_step(
        ...
        if generate_plots:
            self.plot_results(
                ...
                outdir=Path(self.save_dir / "inference_predictions"),
                scores=scores,
            )
    ...
    def unpack_predictions(
            self, predictions: dict, model: NuLite
    ) -> DataclassHVStorage:
...
# CLI
class InferenceNuLiteParser:
    def __init__(self) -> None:
        parser = argparse.ArgumentParser(
            formatter_class=argparse.ArgumentDefaultsHelpFormatter,
            description="Perform NuLite inference for given run-directory with model checkpoints and logs",
        )
