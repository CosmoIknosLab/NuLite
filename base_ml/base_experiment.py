...
class BaseExperiment:
    ...
    def get_scheduler(self, optimizer: Optimizer) -> LRScheduler:
    ...
    def store_config(self) -> None:
        ...
        with open(
            (Path(self.run_conf["logging"]["log_dir"]) / "config_0.yaml").resolve(), "w"
        ) as yaml_file:
        ...
        self.logger.debug(
            f"Stored config under: {(Path(self.run_conf['logging']['log_dir']) / 'NuLite.yaml').resolve()}"
        )
